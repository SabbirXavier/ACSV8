            ...
            <div className="p-6 bg-red-50 dark:bg-red-500/5 rounded-2xl border border-red-200 dark:border-red-500/10 space-y-6">
              <div>
                ...
                <p className="text-sm text-red-500/80 mb-4">
                  Manage users with special roles or active restrictions.
                </p>
                
                <div className="space-y-4">
                  {/* Restricted Users */}
                  <div className="bg-white dark:bg-black/20 p-4 rounded-xl border border-red-200 dark:border-red-500/20">
                    <h5 className="font-bold mb-3 flex items-center gap-2"><ShieldAlert size={16} className="text-orange-500" /> Restricted Users</h5>
                  ...
                  {/* Staff Members */}
                  <div className="bg-white dark:bg-black/20 p-4 rounded-xl border border-blue-200 dark:border-blue-500/20">
                    <h5 className="font-bold mb-3 flex items-center gap-2"><Shield size={16} className="text-blue-500" /> Staff List</h5>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      {chatUsers.filter(u => u.role === 'admin' || u.role === 'moderator').map(u => (
                        <div key={u.id} className="flex justify-between items-center p-2 bg-gray-50 dark:bg-white/5 rounded-lg border border-gray-200 dark:border-white/5">
                          <span className="font-bold text-sm truncate flex-1">{u.name}</span>
                          <span className={`text-[10px] uppercase font-bold px-2 py-0.5 rounded-full ${u.role === 'admin' ? 'bg-purple-100 text-purple-600' : 'bg-blue-100 text-blue-600'}`}>
                            {u.role}
                          </span>
                        </div>
                      ))}
              ...
              <div className="pt-4 border-t border-red-200 dark:border-red-500/20">
                <h4 className="font-bold text-lg text-red-600 dark:text-red-400 mb-3">Bulk Tools</h4>
                <div className="flex flex-wrap gap-3">
                  <button 
                    onClick={() => alert('This would clear all messages in the General channel. Implement via Cloud Function or batch delete.')}
                    className="px-4 py-2 bg-red-500 text-white rounded-xl font-bold hover:bg-red-600 transition-colors"
                  >
                    Clear General Chat
                  </button>
                  <button 
                    onClick={() => alert('This would clear all messages in all channels. Implement via Cloud Function or batch delete.')}
